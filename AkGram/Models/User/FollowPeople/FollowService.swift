...
class FollowService {
    ...
    func fetchCountFollowUser(_ userId: String, completion: @escaping (Int) -> Void) {
        refFollowing.child(userId).observe(.value) { (snapshot) in
            let count = Int(snapshot.childrenCount)
            completion(count)
        }
    }
    
    func fetchCountFollowerUser(_ userId: String, completion: @escaping (Int) -> Void) {
        refFollowers.child(userId).observe(.value) { (snapshot) in
            let count = Int(snapshot.childrenCount)
            completion(count)
        }
    }
